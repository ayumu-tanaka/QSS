...
\begin{frame}[fragile]{CSVファイルの読み込み (1)}
\protect\hypertarget{csvux30d5ux30a1ux30a4ux30ebux306eux8aadux307fux8fbcux307f-1}{}
\begin{itemize}
\tightlist
\item
  \texttt{read.csv()} 関数を用いて、外部のデータセットをRに読み込む。
\item
  原則として、データファイル（\texttt{UNpop.csv}
  など）を作業ディレクトリに保存して読み込むことが推奨される。
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1. ローカルに保存したUNpopデータの読み込み（推奨）}
...
\CommentTok{\# UNpop \textless{}{-} read.csv("https://ayumu{-}tanaka.github.io/QSS/QSS\_Data/UNpop.csv")}

\CommentTok{\# データフレームの次元（行数・列数）を確認}
...
\begin{frame}[fragile]{CSVファイルの読み込み (2)}
\protect\hypertarget{csvux30d5ux30a1ux30a4ux30ebux306eux8aadux307fux8fbcux307f-2}{}
\begin{itemize}
\tightlist
\item
  読み込んだデータの中身を \texttt{head()} で確認します。
\end{itemize}
